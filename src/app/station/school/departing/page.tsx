...
export default function SchoolDepartingPage() {
  ...
  const checkTrip = useCallback(async () => {
    ...
    try {
      ...
      if (data.trip?.status === 'returned') {
        router.push(`/station/school/report?trip_id=${tripId}`);
        return true;
      }
      return false;
    } catch {
  ...
  return (
    <MobileShell>
      <div className="min-h-dvh flex flex-col items-center justify-center px-5">
        ...
        <h1 className="font-title text-h2 text-ink-1">前往学校...</h1>
        <p className="font-title text-body text-ink-3 mt-3">
          小青龙正在去上课的路上{'.'.repeat(dots)}
        </p>
